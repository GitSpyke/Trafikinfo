...
export async function GetNearbyStation(setStationCoord, setDepartures, coordinates) {
    ...
    $(results.TrainAnnouncement).each(function (item) { addToDepartures(stations, departures, item, results) })//destination: results.TrainAnnouncement[item].ToLocation[0].LocationName, 
    setDepartures(departures.slice(0, 5))
}

function addToDepartures(stations, departures, item, results) {
    try {
        departures.push(({ destination: stations[results.TrainAnnouncement[item].ToLocation[0].LocationName], number: results.TrainAnnouncement[item].AdvertisedTrainIdent, time: results.TrainAnnouncement[item].AdvertisedTimeAtLocation.substr(11, 5) }));
    }
    catch {}
}
